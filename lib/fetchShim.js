...
export const createFetchShim = ({ baseUrl, realFetch = globalThis.fetch, ai = "off", aiHook } = {}) => {
  ...
  const shim = async (input, init) => {
    ...
    // A Hub scenario download in progress. The game server runs in THIS process,
    // so its /api/hub/file proxy fetches through here too, and would otherwise
    // be counted as a model call — or refused outright with AI off. Map data,
    // not a model call; open only for the length of the download, and only to
    // the hosts the game's proxy itself allows.
    if (hubHosts.size && hubHosts.has(hostnameOf(url))) {
      stats.hub += 1;
      return realFetch(input, init);
    }
  ...
  shim.stats = stats;
  /** Let `hosts` (the game's hub download hosts) through until close() is called. */
  shim.openHubRoute = (hosts = []) => {
    hubHosts = new Set(hosts.map((host) => String(host).toLowerCase()));
    return () => {
      hubHosts = new Set();
    };
  };
