...
import { describeSaveLines } from "./gameExport.js";
  ...
  try {
    ...
    for (const [index, fixture] of saveList.entries()) {
      ...
      if (session.save.kind === "export") {
        ...
        const lines = describeSaveLines(save);
        report.describeSave({
          lines,
          warnings: save.warnings,
          meta: save.aiPlan?.fromRecord ? { model: save.aiPlan.model } : {},
        });
        for (const line of lines) say(`  ${line}`);
        for (const warning of save.warnings) say(`  ! ${warning}`);
        for (const mismatch of save.importCheck.mismatches) {
          report.add(
            {
              id: "import-mismatch",
              severity: "critical",
            ...
            { step: "importing the Game export" },
          );
          say(`  ! Import finding: ${mismatch.summary}`);
        }
      }
