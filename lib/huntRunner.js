...
import { saveForReport } from "./gameExport.js";
...
export const runBugHunt = async ({
  ...
  // A Game export is one Save, opened inside the session rather than copied in.
  const saveList = saveZip ? [null] : resolveSaves(saves, gameRepo);
  const saveLabel = (fixture) => (saveZip ? `Game export ${path.basename(saveZip)}` : (fixture ?? "fresh scenario"));
  ...
  say(`Saves: ${saveList.map(saveLabel).join(", ")}`);
  ...
  try {
    ...
    for (const [index, fixture] of saveList.entries()) {
      ...
      say(`--- ${saveLabel(fixture)} (${index + 1}/${saveList.length}) ---`);
      report.step(`opening ${saveLabel(fixture)}`);
      ...
      session = await createSession({
        ...
        importScenario,
        hub,
        onProgress: (text) => say(`  ${text}`),
        quiet: true,
        allowWrite: [runsDir, path.join(harnessRoot, "cassettes")],
        aiHook,
      });
      journal.state.target = session.target.describe();
      journal.start({ target: session.target.describe(), level, seed, fixture, saveZip: saveZip ? path.basename(saveZip) : null });

      if (session.save.kind === "export") {
        const forReport = saveForReport(session.save);
        const { aiPlan } = session.save;
        report.describeSave({ ...forReport, meta: aiPlan?.fromRecord ? { model: aiPlan.model } : {} });
        for (const line of forReport.lines) say(`  ${line}`);
        for (const warning of forReport.warnings) say(`  ! ${warning}`);
        for (const finding of forReport.importFindings) {
          report.add(finding, { step: "importing the Game export" });
          say(`  ! Import finding: ${finding.summary}`);
        }
      }
    ...
    // A bad Game export, or a target that cannot import one, is a sentence for
    // the person running this, not a crash in the game.
    inputError = Boolean(error?.harnessExport || error?.harnessCompat);
    if (!inputError) report.crash(error, { context: "the hunt itself failed" });
    report.finish("failed", hideHomeDir(error.message));
    journal.log("error", error.message, { stack: error.stack });
    journal.finalize(RUN_STATUS.failed, { error: error.message });
    exitCode = error?.harnessCompat ? 5 : 2;
  ...
  // A bad input keeps its own exit code; nothing was hunted, so no finding outranks it.
  if (!inputError && (critical || report.state.crashes.length)) exitCode = 1;
  ...
  return {
    ...
    exitCode,
    summary:
      `HUNT level=${level} save=${saveZip ? path.basename(saveZip).replace(/\s/g, "_") : (saveList[0] ?? "fresh")} findings=${findings.length} ` +
