...
const STAND_IN_WARNING =
  "Played on the Stand-in scenario, not the Game's own Scenario. Findings about regions may not be real.";

const driftWarning = (drift) =>
  `The game's hub download has changed since the harness copied it (${drift.reason}); update lib/hubScenario.js.`;
 ...
 *
 * `opened` is readGameZip's result. Returns the new Game's id, the Game bundle as it
 * was actually sent (the import check compares against that), and which Scenario case
 * applied.
 */
export const placeExport = async (
  ...
  const name = ref.scenarioName ?? ref.scenarioId ?? "the Scenario";
  ...
  if (opened.scenarioBundle && importScenario) {
    ...
    map = standIn("embedded-skipped", `The zip carries its Scenario, and --no-embedded-scenario left it out. ${STAND_IN_WARNING}`);
  } else if (ref.builtIn) {
    ...
    try {
      ...
      map = {
        ...
        warning: fetched.drift.ok
          ? null
          : `${driftWarning(fetched.drift)} This Scenario was fetched the old way and may not be what a player gets.`,
      };
    } catch (error) {
      const drifted = error.drift && !error.drift.ok ? ` ${driftWarning(error.drift)}` : "";
      map = standIn(
        "hub-failed",
        `Downloading "${name}" from the community hub failed (${error.message}).${drifted} ${STAND_IN_WARNING}`,
      );
      map.error = error.message;
    }
  } else if (ref.missing) {
    map = standIn("missing", `The player did not have this Game's Scenario either. ${STAND_IN_WARNING}`);
  } else {
    map = standIn(
      "not-carried",
      `The Scenario was not in the zip (too large to travel, or exported before Scenarios did). ${STAND_IN_WARNING}`,
    );
  }
 ...
 * server now serves it is compared with the Game bundle as sent, and the Roll-back
...
/** Each Import finding, in the shape the reports' findings take. */
export const importFindingsOf = (save) =>
  (save?.importCheck?.mismatches ?? []).map((mismatch) => ({
    id: "import-mismatch",
    severity: "critical",
    ...
    detail: {
      ...
      why: "a player importing this Game gets something other than what was exported",
    },
    context: "importing the Game export",
  }));

/**
 * An opened Save as the reports need it: plain data, so a scenario run can
 * journal it and a report rebuilt later, in another process, still has it.
 */
export const saveForReport = (save) => ({
  lines: describeSaveLines(save),
  warnings: save?.warnings ?? [],
  importFindings: importFindingsOf(save),
  settingsText: save?.settings?.text ?? null,
});
...
export const describeSaveLines = (save) => {
  ...
  lines.push(`  Scenario: ${(MAP_WORDS[save.map.kind] ?? (() => save.map.kind))(save.map)}`);
