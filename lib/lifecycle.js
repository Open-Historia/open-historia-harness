...
//
// When the agent driving a run hits a session or weekly limit, this process dies
// without warning. Three things must not happen: an orphaned Node process quietly
// playing turns and burning Gemini quota; a git worktree stranded inside the
// user's game repo; and a run losing everything it had achieved.
//
...
//   2. orphan detection the parent pid stops existing -> the session died
...
export const installLifecycle = ({
  ...
  // The one that actually stops runaway quota burn. If the session dies, this
