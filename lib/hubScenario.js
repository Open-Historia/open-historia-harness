// Hub scenarios: fetching a Game export's Scenario from the community hub.
//
// fetchHubScenario is adapted from downloadHubBundle in Open Historia's
// src/Game/GameUI/communityHub.jsx, © 2026 Nicholas Krol, AGPL-3.0-or-later.
//
 ...
 */
export const fetchHubScenario = async (session, origin, { name = "the Scenario", say = () => {} } = {}) => {
  ...
  let bundle;
  try {
    // Said BEFORE the request: the game's proxy holds its answer until the whole
    // file has arrived, which for a big map is minutes of apparent silence. No
    // size is known yet — a Game export records none for a map it did not carry.
    say(`downloading "${name}" from the community hub; a large Scenario can take a few minutes...`);
    ...
    if (length > 0) say(`downloaded "${name}" (${megabytes(length)})`);
    ...
    // A changed game is a likely reason for a failed download, so the drift
    // check's answer must survive the failure rather than be lost with it.
    error.drift = drift;
    throw error;
  } finally {
    closeHub();
