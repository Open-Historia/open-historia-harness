...
import { createSandbox, SANDBOX_GAME_ID, SANDBOX_SCENARIO_ID, seedSourceDir } from "./sandbox.js";
...
test("stock geometry turns OFF customRegions in the seeded world", { skip: !available }, () => {
  ...
  const source = JSON.parse(fs.readFileSync(path.join(seedSourceDir(target.path), "world.json"), "utf8"));
...
test("writes land in the sandbox and nowhere else", { skip: !available }, async () => {
  ...
  const sourceEvents = fs.readFileSync(path.join(seedSourceDir(target.path), "storage/events.json"), "utf8");
