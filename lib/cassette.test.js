...
// never reaches disk. The user's key is a free-tier key, but a key committed to a repo
...
test("redact catches every Google credential shape, not just AIza", () => {
  // The AIza rule alone missed a real key handed over in the "AQ." form, which
  // only surfaced because a real key was tested against the redactor.
  for (const shape of [
    "AIzaSyD-fake0000000000000000000000000000",
    "AQ.Ab8RN6FAKEFAKEFAKEFAKEFAKEFAKEFAKEFAKE",
    "ya29.a0FAKEFAKEFAKEFAKEFAKEFAKEFAKEFAKE",
  ]) {
    assert.doesNotMatch(redact(`the key is ${shape} ok`), new RegExp(shape.slice(0, 12)), shape);
  }
});
