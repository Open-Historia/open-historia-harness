...
import { meterResponse } from "./usageMeter.js";
...
export const createAiHook = ({
  ...
  // A Game export's Settings record can name the model after the hook exists,
  // and cassettes are keyed on it, so it can be changed once the Save is open.
  let currentModel = model;

  const hook = async ({ url, init, realFetch, task }) => {
    const request = { url, body: init?.body, model: currentModel, task };
    ...
    return paced(async () => {
      ...
      for (;;) {
        ...
        if (response.status !== 429 && response.status < 500) {
          ...
          // Measured off a clone, never awaited: the game streams, and blocking
          // the response here would serialise generation behind measurement.
          meterResponse({
            response,
            init,
            url,
            ...
            emit: (usage) => {
              onUsage?.(usage);
              journal?.log("info", `provider usage for ${task ?? "task"}`, { wire: usage });
            },
          });
