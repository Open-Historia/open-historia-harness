...
export const createSandbox = ({
  ...
  if (fixture) {
    ...
    // cpSync copies MODE as well as content, so a source deliberately marked
    // read-only — the sane way to hold a copy of someone's real save — produces
    // a sandbox the engine cannot write its first turn into. The protection
    // belongs on the source; the copy has to be writable to be played.
    for (const entry of fs.readdirSync(to, { recursive: true, withFileTypes: true })) {
      ...
      try {
        ...
        // A filesystem without modes: nothing was read-only to begin with.
      }
    }
