...
export const createSandbox = ({
  ...
  const source = seedSourceDir(target.path);
  if (!fs.existsSync(path.join(source, "world.json"))) {
    throw new Error(
      `[harness sandbox] no seed scenario at ${source}. The target checkout looks incomplete.`,
    );
  }
  ...
  if (fixture) {
    ...
    if (instance) {
      writeJson(instancePath, { ...instance, scenarioId: SANDBOX_SCENARIO_ID });
    }
