...
test("a scenario opens on a Game export by short name, and says what it opened", { skip }, () => {
  ...
  assert.match(out, /Map: its own Scenario "Harness Sandbox", carried in the zip/);
...
test("a map the zip carries under a new id is imported for the Game to play on", { skip }, async () => {
  ...
  const { code, out } = run(["smoke", "--save-zip", file]);
  assert.equal(code, 0, out);
  assert.match(out, /Map: its own Scenario "Harness Sandbox", carried in the zip$/m);
...
test("--no-embedded-scenario plays on the Stand-in instead, and warns", { skip }, () => {
  ...
  assert.equal(code, 0, out);
  assert.match(out, /Map: the Stand-in scenario: the zip carries "Harness Sandbox", but --no-embedded-scenario left it out/);
...
test("a Game on a built-in map plays on the Stand-in without a warning", { skip }, async () => {
  ...
  const { code, out } = run(["smoke", "--save-zip", file]);
  assert.equal(code, 0, out);
  assert.match(out, /Map: the Stand-in scenario, standing in for the built-in "Modern Day"/);
...
test("a Game whose map the player never had plays on the Stand-in, and warns", { skip }, async () => {
  ...
  const { code, out } = run(["smoke", "--save-zip", file]);
  assert.equal(code, 0, out);
  assert.match(out, /Map: the Stand-in scenario: the player did not have "Lost Map" either/);
  assert.match(out, /WARNING: The player did not have this Game's map either/);
});
...
test("a hub map left undownloaded by --no-hub plays on the Stand-in, and warns", { skip }, async () => {
  ...
  assert.equal(code, 0, out);
  assert.match(out, /Map: the Stand-in scenario: "Hub Map" is on the community hub, and it was not downloaded/);
...
test("--check-exports opens every export in the folder without playing, and says how each went", { skip }, () => {
  ...
  try {
    ...
    assert.match(out, /^\s+harness\/round-trip\s+opens · import ok · 1 Roll-back point · map: embedded · \d+ findings already in the Save$/m);
    assert.match(out, /^\s+null-events\s+opens · 1 IMPORT FINDING/m);
    assert.match(out, /^\s+hub-map\s+opens · import ok · 1 Roll-back point · map: hub-skipped/m);
