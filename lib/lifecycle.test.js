...
//
// When the agent driving a run hits a session or weekly limit, the run must
...
test("SIGKILL leaves a run that the NEXT startup recovers", async () => {
  ...
  // clean up — exactly what a session limit does to the agent driving a run.
  const id = "killed";
  const script = `
  ...
  // A literal runbook, so whoever picks the run up next knows exactly what to type.
