...
export const createDriver = async (session, { catalog } = {}) => {
  ...
  const observed = async (task, run) => {
    ...
    // aiByTask counts for the WHOLE run, so the count has to be read as a delta.
    // Taken as an absolute it made every later turn of the same verb report the
    // sum of all its predecessors — a five-turn run showed 2,4,6,8,10 for five
    // two-call turns, and the report's AI-call total was 35 where the provider
    // had answered 15. On a branch comparison that inflates the busier branch
    // the harder, which is exactly backwards.
    const callsBefore = session.fetch.stats.aiByTask[task] ?? 0;
    ...
    return {
      ...
      providerCalls: (session.fetch.stats.aiByTask[task] ?? 0) - callsBefore,
    };
  };
