...
import { harnessExportsDir, saveForReport } from "./gameExport.js";
...
// fileURLToPath, not URL.pathname, which keeps the URL encoding: with a space in
// the checkout's path, --list found no scenarios at all. See roots.test.js.
...
/**
 * Everywhere a scenario run may write outside its sandbox: the journal and
 * reports for this run, the cassette store, both of which must outlive the
 * sandbox, and the one part of game-exports/ the harness writes — never the
 * player zips above it.
 */
export const runWritableRoots = () => [RUNS_DIR, path.join(HARNESS_ROOT, "cassettes"), harnessExportsDir()];
...
export const runScenarios = async (names, options = {}) => {
  ...
  try {
    const cassette = createCassette({
      dir: path.join(HARNESS_ROOT, "cassettes", cassetteName ?? "default"),
      mode: cassetteMode,
      strict: replayMode === "strict",
    });
    const quota = createQuota({
      file: path.join(sandboxRoot, "quota.json"),
      perDay: Number(maxAiCallsPerDay ?? 0),
      perRun: Number(maxAiCalls ?? 0),
    });
    // The 6.5 s default paces a fresh scenario's ~32k-token prompts comfortably
    // inside a free tier. A real save is a different animal: a 1939-event game
    // sends 86k tokens a call, and nine calls a minute of that is far past any
    // free-tier tokens-per-minute ceiling — which arrives as a 429, becomes a
    // fallback turn, and quietly ruins the run it was supposed to measure.
    const aiHook = createAiHook({
      cassette,
      quota,
      journal,
      model: sessionOptions.model ?? "",
      ...(minGapMs ? { minGapMs: Number(minGapMs) } : {}),
    ...
    session = await createSession({
      ...
      sandboxRoot,
      quiet: true,
      allowWrite: runWritableRoots(),
      aiHook,
      ...sessionOptions,
    });
    runQuota = quota;
    runCassette = cassette;
    ...
    // What the run was opened on. For a Game export, a failed import check is an
    // assertion failure in its own right: data the importer lost is a bug a
    // player hits the moment they import, whatever the scenarios then find.
    if (session.save.kind === "export") {
      // Journalled once, so report.md shows it at the top however the run ends.
      const forReport = saveForReport(session.save);
      journal.save(forReport);
      writeReport(journal.runDir);
      for (const line of forReport.lines) say(line);
      for (const warning of forReport.warnings) say(`WARNING: ${warning}`);
      for (const finding of forReport.importFindings) say(`WARNING: Import finding: ${finding.summary}`);
      expect.that(session.save.importCheck.ok, "the Game export imports as it was exported (Import findings otherwise)", {
        mismatches: session.save.importCheck.mismatches,
      });
    }
    ...
    // Being rate limited is not being broken. Conflating them would make every
    // quality number in the report untrustworthy, so it gets its own exit code.
    const quotaProblem = error instanceof QuotaExhausted || error?.harnessQuota;
    journal.finalize(RUN_STATUS.failed, { error: error.message, quota: quotaProblem });
    // A target that cannot run what was asked (compat shim, or no Game exports) is 5.
    exitCode = quotaProblem ? 3 : error?.harnessCompat ? 5 : 2;
    say(quotaProblem ? `QUOTA: ${error.message}` : `ERROR: ${error.message}`);
  } finally {
    await shutdown();
  }
  ...
  return {
    ...
    summaryLine,
    recovered,
    quota: runQuota ? { spentThisRun: runQuota.spentThisRun, spentToday: runQuota.spentToday() } : null,
    cassette: runCassette?.stats ?? null,
  };
};
