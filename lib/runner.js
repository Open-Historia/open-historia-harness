...
import { describeSaveLines, harnessExportsDir } from "./gameExport.js";
...
export const runScenarios = async (names, options = {}) => {
  ...
  try {
    ...
    if (session.save.kind === "export") {
      for (const line of describeSaveLines(session.save)) log.info(line);
      for (const warning of session.save.warnings) log.warn(warning);
      for (const mismatch of session.save.importCheck.mismatches) log.warn(`Import finding: ${mismatch.summary}`);
