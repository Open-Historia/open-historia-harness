...
test("a checkout under a path with a space, % or # still finds its own directories", async () => {
  ...
  // The copy sits outside this checkout, so its package.json no longer applies.
  // Without one of its own, Node would take the module type from syntax
  // detection or from any package.json above the temp dir, and a stray
  // "type": "commonjs" there would fail the import for reasons unrelated to paths.
