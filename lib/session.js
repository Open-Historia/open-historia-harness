...
export const createSession = async ({
  ...
  // 3-5.
  installFsGuard();
  const sandbox = createSandbox({ target, geometry, fixture, fixtureSource });
  ...
  // Without this an "openai-compatible" run is pointed at the default
  // localhost:11434 and every call leaves for a gateway that is not there —
  // which is how NVIDIA, OpenRouter and every other hosted gateway looked
  // broken when they were simply never given an address.
  if (endpoint) settings[`${provider.replace(/-/g, "_")}_endpoint`] = endpoint;

  // An OpenAI-shaped gateway sends no `usage` block on a STREAMED response
  // unless the request asks for one, and the game streams everything. Without
  // this every NVIDIA/OpenRouter/Groq run reports null tokens and cannot be put
  // beside a Gemini run at all. It goes through the game's own custom-parameters
  // setting — the same field a player fills in — rather than being spliced into
  // the request behind the engine's back, and an explicit value wins.
  if (provider.endsWith("compatible")) {
    const key = `${provider.replace(/-/g, "_")}_custom_params`;
    if (!settings[key]) {
      settings[key] = JSON.stringify({ stream_options: { include_usage: true }, ...(customParams ?? {}) });
    }
  }

  // Any other setting the engine reads from localStorage, for a run that needs a
  // player switch thrown — `OH_HARNESS_STORAGE={"ai_save_requests":"0"}`. It is
  // the only way to compare a branch against itself with one feature off, and
  // there is no UI in this process to throw the switch with.
  try {
    const extra = JSON.parse(process.env.OH_HARNESS_STORAGE ?? "{}");
    for (const [key, value] of Object.entries(extra)) settings[key] = String(value);
  } catch {
    // A malformed override is ignored rather than failing the run: the settings
    // it would have carried are all defaults anyway.
  }
